...
//Player Core structure
    bool Game_Player::Get_Player_Status()
    {
        if(map->Get_Player_Life_Left()==0)
        {
            return true;
        }
        else
    ...
    void Game_Player::Current_Player_Attack_Enemy(Game_Player& current_player, Game_Player& enemy_player )
    {
        int x;
        int y;
        bool targeted_area;
        ...
        x=user_input();
        y=user_input();
        targeted_area= enemy_player.Get_Enemy_didHit(y,x);
    ...
    bool Game_Player::Get_Enemy_didHit(int y, int x)
    {
        int contained_ship;
